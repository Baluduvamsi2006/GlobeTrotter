...
export default function TripCard({ title, startDate, endDate, totalBudget }: TripCardProps) {
  ...
  return (
    <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6 mb-4 transition-all duration-200 hover:shadow-md hover:border-sky-300">
      ...
      <div className="flex flex-col sm:flex-row gap-4">
        ...
        <div className="inline-flex items-center gap-2 px-4 py-2 rounded-lg bg-slate-50 border border-slate-200 text-slate-700">
          ...
          <span className="font-medium text-sm">Budget: {totalBudget ? `$${totalBudget.toFixed(2)}` : 'TBD'}</span>
        </div>
      </div>
    </div>
  );
